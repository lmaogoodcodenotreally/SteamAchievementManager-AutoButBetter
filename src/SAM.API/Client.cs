...
public class Client : IDisposable
{
    ...
    public void Initialize(uint appId)
    {
        ...
        try
        {
            ...
            if (_pipe == default) throw new ClientInitializeException(ClientInitFailure.CreateSteamPipe);

            _user = SteamClient.ConnectToGlobalUser(_pipe);
            if (_user == default) throw new ClientInitializeException(ClientInitFailure.ConnectToGlobalUser);

            SteamUtils = SteamClient.GetSteamUtils004(_pipe);
            if (appId > 0 && SteamUtils.GetAppId() != appId) throw new ClientInitializeException(ClientInitFailure.AppIdMismatch);
        ...
        catch (Exception ex)
        {
            try
            {
                var logsDir = System.IO.Path.Combine(AppContext.BaseDirectory ?? Environment.CurrentDirectory, "logs");
                System.IO.Directory.CreateDirectory(logsDir);
                var path = System.IO.Path.Combine(logsDir, $"client_initialize_error_{DateTime.UtcNow:yyyyMMdd_HHmmss}.log");
                var msg = $"Client.Initialize failed for AppId={appId}\nException: {ex}\n";
                System.IO.File.WriteAllText(path, msg);
                log.Error($"Client.Initialize failed, dumped details to {path}", ex);
            }
            catch
            {
