...
public partial class HomeViewModel
{
    ...
    public HomeViewModel(HomeSettings settings = null, SteamUser user = null)
    {
        User = user;
    ...
    private void LoadView(LibraryView type)
    {
        ...
        try
        {
            ...
            if (type == LibraryView.Tile)
            {
                ...
                var vm = new LibraryTileViewModel(tileSettings, User);
            ...
            if (type == LibraryView.DataGrid)
            {
                ...
                var vm = new LibraryDataGridViewModel(gridSettings, User);
    ...
    private void OnActionMessage(ActionMessage obj)
    {
        if (obj.EntityType == EntityType.HomeSettings && obj.ActionType == ActionType.Changed)
        {
            ...
            // Ensure the current library view picks up setting changes (grouping, show-images)
            try
            {
                CurrentVm?.Refresh();
            }
            ...
            {
                log.Error($"Failed to refresh current view after settings change: {e.Message}", e);
            }
        }
    }
